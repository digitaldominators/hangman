...
class GameViewSet(viewsets.GenericViewSet):
    ...
    * other_player_game_score - int - current score of the other players game
    ...
    def create(self, request):
        ...
        if request.data.get('multiplayer'):
            if request.data.get('category_text', "") == "":
                return Response({'message': 'Category cannot be blank'}, status=status.HTTP_400_BAD_REQUEST)
            elif request.data.get('word', "") == "":
                return Response({'message': 'Word cannot be blank'}, status=status.HTTP_400_BAD_REQUEST)
