...
class ScoreboardViewSet(viewsets.GenericViewSet):
    def list(self, request, *args, **kwargs):
        total_scores = UserProfile.objects.filter(score__gt=0, private=False).order_by(
            "-score"
        )[:50]
        average_scores = UserProfile.objects.filter(
            avg_score__isnull=False, private=False
        ...
        return Response(
            {
                "total_scores": total_scores_serializer.data,
